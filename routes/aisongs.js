...
router.post('/aisongs', isAuthenticated, upload.single('song'), (req, res) => {
  ...
  db.run(insertSql, [userId, songPath, title, submittedBy, uploadedAt], function (err) {
    if (err) {
    ...
    db.run(updatePointsSql, [pointsToAdd, userId], (err3) => {
      if (err3) {
        console.error('Error updating user points:', err3);
        // Optional: continue even if points fail
      } else {
